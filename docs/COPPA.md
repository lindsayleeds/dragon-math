...
3. **Server is empty for guests.** `psql "$DATABASE_URL" -c "SELECT id, username, account_type FROM users;"` should show only the parent + previously-linked kids. The guest does not appear.
